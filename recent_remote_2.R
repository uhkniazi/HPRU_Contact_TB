...
## load the data
dfSamples = read.csv('Data_external/Recent_remote2/Groups.txt', header=F, sep='\t')
load('Data_external/Recent_remote2/data_matrix.R')
dfData = data.frame(selDataMatrix)
rm(selDataMatrix)
...
# remove samples followup and indeterminate
i = which((dfSamples$V2) %in% c('Followup', 'Indeterminate'))
...
table(rownames(mData) %in% dfSamples$V1)
# remove these missing samples
f = (rownames(mData) %in% dfSamples$V1)
mData = mData[f,]
table(rownames(mData) %in% dfSamples$V1)

i = match(rownames(mData), dfSamples$V1)
dfSamples = dfSamples[i,]

# create grouping factor
fSamples = as.character(dfSamples$V2)
fSamples = factor(fSamples, levels = c('Remote', 'Recent', 'Resistant'))
table(fSamples)
head(fSamples)
dfSamples$fSamples = fSamples
names(fSamples) = dfSamples$V1
## sanity check
table(rownames(mData) %in% dfSamples$V1)
...
# pheno data
rownames(dfSamples) = dfSamples$V1
...
write.csv(df.rn[,-1], file=paste('Results/', 'de_only_in_resistant_vs_remote', '.csv', sep=''))
